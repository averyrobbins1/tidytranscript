#' Scrape SAT scores from a student's transcript
#'
#' @param .data A character vector returned from the read_transcript function.
#'
#' @return A tibble of SAT test scores
#' @export
#'
#' @examples
#' \dontrun{
...
scrape_sat <- function(.data) {
    ...
    dat2 %>%
        tibble::enframe() %>%
        dplyr::mutate(test = stringr::str_detect(value, "\\bSAT$")) %>%
        ...
        dplyr::mutate(date = stringr::str_extract(value, '\\d{2}\\/\\d{2}\\/\\d{2}'),
               test = stringr::str_extract(value, 'SAT$'),
        ...
        tidyr::separate(info, into = c('WRIT','MATH', 'CRIT_READ'), sep = " ") %>%
        dplyr::filter(WRIT != "WRIT") %>%
        tidyr::pivot_longer(cols = c(WRIT, MATH, CRIT_READ),
